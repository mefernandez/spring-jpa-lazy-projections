...
public class EmployeeRestControllerTest {
	...
	@Test
	public void lazyFetchTypeSerializesBossName() throws Exception {
		this.mvc.perform(get("/lazy/employees")
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.numberOfElements").value("20"))
	...
	@Test
	public void lazyFetchTypeDoesNotSerializesDepartmentName() throws Exception {
		this.mvc.perform(get("/lazy/employees")
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.numberOfElements").value("20"))
	...
	@Test
	public void lazyFetchTypeQueriesDepartmentTableZeroTimes() throws Exception {
		this.mvc.perform(get("/lazy/employees")
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
		
		assertEquals(0, count("select .* from department", systemOutRule.getLog()));
	}
	...
	public void lazyFetchTypeTotalQueries() throws Exception {
		...
		assertEquals(2, count("select .* from", systemOutRule.getLog()));
	}
	...
	public void lazyFetchJSONTotalCharacters() throws Exception {
		...
		assertEquals(4574, response.getResponse().getContentAsString().length());
	}
